...
func Example_map_key_struct() {
	p1 := person{name: "foo", age: 20}

	Display("x", map[person]bool{p1: true})
	// Output:
	// Display x (map[display.person]bool):
	// x[{name: "foo", age: 20}] = true
}
func Example_map_key_array() {
	arr := [3]int{1, 2, 3}

	Display("x", map[[3]int]bool{arr: true})
	// Output:
	// Display x (map[[3]int]bool):
	// x[{1, 2, 3}] = true
}
